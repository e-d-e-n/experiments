...
!(function(t, e) {
	...
	function h(t) {
		function e() {
			n.offsetWidth != r.offsetWidth && 0 != n.offsetWidth
				? requestAnimationFrame(e)
				: (document.body.removeChild(n),
				  document.body.removeChild(r),
				  f.hasTouch && E.classList.add('disable-touch-select'),
				  (function(t) {
						var e = document.createElement('div')
						e.style.cssText = 'display:block;white-space:pre;position:absolute;'
						e.innerHTML = 'X'.repeat(100) + '\n'
						e.innerHTML += 'X\n'.repeat(99)
						document.body.appendChild(e)
						var n = e.offsetWidth / 100
						var r = e.offsetHeight / 100
				  ...
				  document.addEventListener('touchstart', function(t) {
						;(T.x = t.touches[0].clientX),
							(T.y = t.touches[0].clientY),
							(T.px = T.x),
							(T.py = T.y)
				  }),
	...
	function l(e) {
		...
		for (
			...
			c(q, D, O, 4),
			(F.canvas.width = O),
			(F.canvas.height = Math.ceil(D / X.aspect)));
			E.childNodes.length > D;

		)
			E.removeChild(E.lastChild)
		for (; E.childNodes.length < D; )
		...
		;(B += 0.05 * (h - B)) > 0 &&
			d.circle(k, o, i, a, Math.floor(B), L.map.get('0')),
			(A.length = 0)
		var f = !0,
			v = !1,
			p = void 0
		...
		F.save(),
			...
			(F.fillStyle = 'black')
		var b = Math.max(w, x),
			C = t(Math.cos(0.0011 * e), -1, 1, 0.085 * b, 0.85 * b),
				...
				: 2 == o
					...
					: 3 == o
						? r(t, function(t, e) {
								return ~~(
									Math.sqrt(
										Math.pow(i - t, 2) + Math.pow((a - e) / X.aspect, 2)
									) / 2
								)
						  })
						: 4 == o &&
						  (w.seed(Math.random()),
						  r(t, function(t, e) {
								return ~~(120 * w.perlin2(0.03 * t, (0.03 * e) / X.aspect))
						  }))
	}
	function u(t, e, n) {
	...
	var s = {
			resize2d: function(t, e, n, r) {
				...
				for (var o = 0; o < t.length; o++)
					...
					else for (var i = t[o].length; i < n; i++) t[o][i] = r
		...
		d = {
			image: function(e, n, r) {
				for (
					var o = r.canvas,
						i = r.getImageData(0, 0, o.width, o.height).data,
						a = Math.min(o.width, e[0].length),
						h = Math.min(o.height, e.length),
						l = n.chars.length,
						c = n.chars.map.get('1'),
						u = 0;
					u < h;
					...
					for (var s = u * o.width, f = 0; f < a; f++) {
						...
						i[d++],
							...
							v < 255 &&
								(f + (u % 2)) % 2 == 1 &&
								(e[u][f] = Math.round(t(v, 0, 255, c, l - 1)))
					}
			},
			circle: function(t, e, n, r, o, i) {
				for (
					var a = e.metrics.aspect,
						h = o,
						l = Math.floor(o * a) + 1,
						c = h * h,
						u = Math.max(n - h, 0),
						s = Math.max(r - l, 0),
						f = Math.min(n + h + 1, t[0].length),
						d = Math.min(r + l + 1, t.length),
						v = s;
					v < d;
					v++
				)
					for (var p = u; p < f; p++)
						Math.pow(p - n, 2) + Math.pow((v - r) / a, 2) < c &&
							(p + (v % 2)) % 2 == 0 &&
							(t[v][p] = i)
			},
			textBox: function(t, e, n, r, o, i, a) {
				for (
					...
					if ('br' == v.tag) (l += 1), (h = r)
						...
						var m = v.length
						p && p.punctuation && (m += p.length),
							h + m > u && ((h = r), (l += 1))
						for (var g = 0; g < v.length; g++){
							s.set(t, l, h + g, e.chars.map.get(v.text[g]))
						}
						('a' == v.tag || 'b' == v.tag) &&
							l >= 0 &&
							l < c &&
	...
	window._enterFullScreen = () => screenfull.request(E)
	gui.add(window, '_enterFullScreen').name('enter fullscreen')

	var C =
			'aáàâãbcçdeèéêfghiîìïíjklmnoóòõôpqrstuúùûüvwxyz%/|\\()#?!"\'“”‘’;:&π*+•=—-_,. ',
		L = {
			list: Array.from(C),
			map: new Map(
				Array.from(C).map(function(t, e) {
					return [t, e]
				})
			),
			length: C.length,
				...
				b(t, [
						...
						value: function() {
							this.tokens = (function(t) {
								var e = [],
									...
									r = null
								return (
									t.replace(/<([^\s>]*)[^>]*>|[^\s<]*/g, function(t, o, i) {
										o
											? ('/' == o[0]
													? ((n = null), (r = null))
													: ((n = o.toLowerCase()), (r = t)),
					...
					},
					{
						...
						value: function(t) {
							...
							return Math.round(e.left / t.charWidth) + options.offsetX
						},
					},
					{
						key: 'y',
