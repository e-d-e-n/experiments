...
!((new BroadcastChannel('frame-faces')).onmessage = ({data}) => animate(faces2 = data))
...
const animate = () => {
	...
	for(var i = 0; i < faces2.length; i++){
		...
		imageDataCtx.strokeStyle = '#006'
		imageDataCtx.strokeRect(
			(face.x - face.scale / 2) -1,
			(face.y - face.scale / 2) -1,
			face.scale, face.scale,
		)

		imageDataCtx.strokeStyle = '#606'
		imageDataCtx.strokeRect(
			(face.x - face.scale / 2) -1,
			(face.y - face.scale ) -1,
			face.scale, face.scale * 2,
		...
		const {progress = 0} = face
		imageDataCtx.save()
		imageDataCtx.translate(faceRect.centerX, faceRect.centerY)
		imageDataCtx.rotate(faceRect.rotation2d)
		imageDataCtx.fillStyle = `rgba(255, 0, 0, ${0.25 * progress})`
