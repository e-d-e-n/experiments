...
const ditherCtx    = $dither.getContext('2d')
...
const trackFaces = async () => {
	...
	if(paused) return
	ditherCtx.drawImage($webcam, 0, 0, resolution.width, resolution.height, 0,0, canvasSize.width, canvasSize.height)
	let image = ditherCtx.getImageData(0,0, canvasSize.width, canvasSize.height)
	ditherCtx.putImageData(CanvasDither.bayer(image, 160), 0, 0)
	ditherCtx.fillStyle = 'rgba(0, 0, 0, 0.75)'
	ditherCtx.fillRect(0, 0, canvasSize.width, canvasSize.height)
...
// setup
!(async () => {
	$webcam.srcObject = await window.navigator.mediaDevices.getUserMedia({video: {width: 640, height: 480}})
	$webcam.play()
	resolution = await dimensionsAvailable($webcam)
	canvasSize = {}
	$dither.width = canvasSize.width = (window.innerWidth/2)
	$dither.height = canvasSize.height = (window.innerHeight/2)
